...
const DIABETIC = ['diabetyk24', 'HEROKU', 'Aero-Medika', 'sugarcubes'];
const TOOLS_SHOPS = ['MROWKA', 'GRANAT'];
const GAMES = ['GOGcomECOM', 'Steam', 'STEAM', 'PlayStation'];
const MEDIA = ['YouTubePremium', 'rp.pl', 'Netflix', 'NETFLIX', 'Google Play', 'help.max.com', 'YouTube', 'NBA League Pass'];
const ORANGE = ['FLEX'];
const CLOTHS = ['STRADIVARIUS', 'zalando', 'miluba.pl', 'smyk', 'SECRET', 'SINSAY', 'kappahl', 'MEDICINE', 'HOUSE', 'RESERVED', 'HM POL', 'GALANTERIA ODZIEZOWA', 'HEBE', 'CROPP', 'vinted'];
const CAR_SHOWER = ['WIKON', 'Myjnia'];
const SHOES = ['CCC', 'e-cizemka', 'ccc.eu', 'eobuwie', 'zapato'];
const COSMETICS = ['ROSSMANN', 'SZALATA CHLEBOWSKA'];
const EMPIK = ['EMPIK'];
const RESTAURANT = ['PIZZERIA LUCA', 'STACJA CAFE', 'CAFE SAN-REMO', 'GRYCAN LODY OD POKOLEN', 'TOMASZ KUROS', 'ZIELONA GORA BW SPOLKA Z O.O.', 'MOCCA', 'KARMEL', 'SLOW FOOD', 'Verde', 'EWA DA', 'STARA PIEKARNIA', 'MCDONALDS', 'TCHIBO', 'PIJALNIA KAWY I CZEKO', 'KUCHNIE SWIATA', 'HEBAN', 'Ohy', 'KRATKA', 'Wafelek i Kulka', 'CIACHOO', 'PIERINO', 'CAFFETTERIA GELATERIA'];
